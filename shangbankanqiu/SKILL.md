...
# 足球 match_type 可先按 providers.football.priority 尝试已启用且有 api_key 的 provider。
# 缺失、关闭、无 key 或失败时，provider fallback 必须落回既有 web_search/webfetch 路径。
# Task 2 会展开 provider 协议细节；本阶段不改变默认 web 行为。

# ========== 首次诊断 (只跑 1 次) ==========
raw     = fetch_score_raw(match_type, follow, config)   # 足球 provider 可选；默认/兜底见 §build_query
...
while poll_idx < config.max_polls:
    ...
    # ---- 第 1 步: SEARCH ----
    try:
        raw   = fetch_score_raw(match_type, follow, config)  # 足球 provider 可选；失败回退 web_search
