        ...
        for m in fresh:
            ...
            try:
                ...
                raw_events = llm_extract_events(
                    ...
                    cap       = 2 * config.max_events_per_msg,        # 过取 2× 防丢
                )
                m["events"] = raw_events
            except Exception:
...
```text
{
  ...
  "events": [                              # 关键事件列表；本场比赛在搜索结果里能识别到的事件
    {
      "id_hash": "<sha1(match_key + ts + text)[:12]>",  # 去重键，REQUIRED；算法见下方
...
id_hash   = sha1(match_key + ts + text).hexdigest()[:12]
```
...
for match in matches:
    ...
    if pbp_url is None:
        match.events = []
        log("no play-by-play source for {match.home} vs {match.away}, skip Stage B")
        continue
...
# Step 2: 对每个有 URL 的 match：
    ...
    raw_events = llm_extract_events(
        page_md,
        match_key = match.home + "|" + match.away,
    ...
    # 2d. id_hash 计算严格复用 §parse_search_result 已规定的算法：
    #     id_hash = sha1(match_key + ts + text).hexdigest()[:12]
    #     四家 Agent 必须用同一份算法，跨 Agent 同事件 → 同 hash，下游去重才一致。
